...
class Manager(object):
    ...
    def review_file(self, classifier_names, data_set):
        classifications_dic = defaultdict([])
        classifications_dic['all'] = []
        for classifier_name in classifier_names:
            if self.classifiers[classifier_name]['info']['settings']:
                ...
                classifications = config.get("Classifier", "classes").split(",")
                for classification in classifications:
                    classifications_dic[classifier_name].append(classification)
                    classifications_dic['all'].append(classification)
        auto_default_class = filter(lambda a: data_set.find(a) > -1, classifications_dic['all'])
        if len(auto_default_class) == 1:
            ...
            while isclass == False:
                ...
                isclass = classifications_dic['all'].find(default_class) > 0
        for classifier in classifications_dic.keys():
            n_reviewed = self.classifiers[classifier]['info']['reviewed']
            classifications_dic[classifier]['current'] = n_reviewed
