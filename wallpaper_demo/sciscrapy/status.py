...
class Status(object):
    ...
    def __init__(self):
        ...
        if len(cur_dirs) == 1 and cur_dirs.count("data") == 0: #In project level folder
            data_dir = "{1}{0}{2}{0}{3}".format(os.sep, os.curdir, cur_dirs[0], "data")
            project_dir = "{0}".format(cur_dirs[0])
        else:# One level into project
            data_dir = "{0}".format("data")
            project_dir = ""
        #Find item
        print "data", data_dir, "project", project_dir
        self.management_ready = False
        self.crawl_ready = False
        self.item = None
        try:
            f, filename, description = imp.find_module("items", [project_dir])
        ...
        print data_dirs
        self.classifiers = {d:{}  for d in data_dirs if isdir(join(data_dir,d))} #initialize all classifiers in dictionary form
        print self.classifiers
        self.classify_ready=False
        for classifier in self.classifiers.keys():
            classifier_dir = join(data_dir, classifier)
